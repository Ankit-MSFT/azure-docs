...
When inheriting permissions from parent items in POSIX-style model, the owner of a child item inherits the owner permission of the parent item. Even if the entity crating the child item has separate named entry for ACL permissions on parent, when inheriting the permissions on child, as an owner, the owner permissions of the parent will apply.
